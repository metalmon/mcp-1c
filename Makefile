.PHONY: build test clean release mock install install-generic install-buh30 run-1c-dev run-mcp run-mcp-dev run-mcp-business run-mcp-business-readonly run-mcp-business-generic run-mcp-business-buh30 run-mcp-dump run-mcp-reindex check-http dump-config cursor-install help
.DEFAULT_GOAL := help
...
install:
	$(MCP_RUN) --install "$(INFOBASE_PATH)" --install-profile "$(INSTALL_PROFILE)" --db-user "$(DB_USER)" --db-password "$(DB_PASSWORD)"

install-generic:
	$(MCP_RUN) --install "$(INFOBASE_PATH)" --install-profile generic --db-user "$(DB_USER)" --db-password "$(DB_PASSWORD)"

install-buh30:
	$(MCP_RUN) --install "$(INFOBASE_PATH)" --install-profile buh_3_0 --db-user "$(DB_USER)" --db-password "$(DB_PASSWORD)"
...
run-mcp: build
	$(MCP_RUN) --base "$(BASE_URL)" --toolset "$(TOOLSET)" --profile "$(PROFILE)" --access "$(ACCESS)"

run-mcp-dev: build
	$(MCP_RUN) --base "$(BASE_URL)" --toolset developer --profile auto --access read_write

run-mcp-business: build
	$(MCP_RUN) --base "$(BASE_URL)" --toolset business --profile auto --access read_write

run-mcp-business-readonly: build
	$(MCP_RUN) --base "$(BASE_URL)" --toolset business --profile auto --access read_only

run-mcp-business-generic: build
	$(MCP_RUN) --base "$(BASE_URL)" --toolset business --profile generic --access read_write

run-mcp-business-buh30: build
	$(MCP_RUN) --base "$(BASE_URL)" --toolset business --profile buh_3_0 --access read_write

run-mcp-dump: build
	$(MCP_RUN) --base "$(BASE_URL)" --dump "$(DUMP_DIR)" --toolset "$(TOOLSET)" --profile "$(PROFILE)" --access "$(ACCESS)"

run-mcp-reindex: build
	$(MCP_RUN) --base "$(BASE_URL)" --dump "$(DUMP_DIR)" --reindex --toolset "$(TOOLSET)" --profile "$(PROFILE)" --access "$(ACCESS)"
...
help:
	...
	@echo   make install-generic    - install generic profile extension
	@echo   make install-buh30      - install buh_3_0 profile extension
	...
	@echo   make run-mcp-business-readonly - run MCP with business tools in read-only mode
	@echo   make run-mcp-business-generic  - run MCP business tools with generic profile
	@echo   make run-mcp-business-buh30    - run MCP business tools with buh_3_0 profile
	...
	@echo   make install INSTALL_PROFILE=buh_3_0
	...
	@echo   make run-mcp TOOLSET=business PROFILE=buh_3_0 ACCESS=read_only
	...
	@echo   CURSOR_ACCESS=$(CURSOR_ACCESS)
