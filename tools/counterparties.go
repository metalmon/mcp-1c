...
func ReadCounterpartiesTool() *mcp.Tool {
	return &mcp.Tool{
		...
		Description: "Инструмент 1С: получить контрагентов из справочника: список с поиском по краткому и полному наименованию/ИНН " +
			"(в списке возвращаются ИНН, КПП и вид, если они заполнены в базе) или один элемент по code/ref/inn+kpp.",
		InputSchema: json.RawMessage(`{
			"type":"object",
			"properties":{
				"search":{"type":"string","description":"Подстрочный поиск по краткому и полному наименованию (Наименование и НаименованиеПолное) или ИНН (без учёта регистра; «» в наименовании не мешают совпадению)"},
